    ...
    success: function(data) {
      ...
      var dragAndSwap = new DragAndSwap({
        ...
        onChange: function(boxes) {
          ...
          $.ajax({
            url:
              "/split_line/" + event_id + "/race/" + $("#hand_dropdown").val(),
            method: "patch",
...
var searchTable = function(element) {
  ...
  if (data == "ทั้งหมด") data = "";
  tb_member.search(data).draw();
};

var timeScoreToggle = function(element) {
  if ($(element).hasClass("is-active")) return false;
...
$(document).ready(function() {
  ...
  $(window).hashchange(function() {
    ...
    if (hash) select = hash.slice(2, hash.length);
    else select = "detail";
  ...
  $.fn.dataTable.ext.search.push(function(settings, data, dataIndex) {
    ...
    if (data[3].replace(/<a.*>.*?<\/a>/gi, "").trim() == race_id) {
      return true;
    }
    return false;
  });
...
$(".button.max").click(function() {
  $(".button.max.is-active").removeClass("is-active");
